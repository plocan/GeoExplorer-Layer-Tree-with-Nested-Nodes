        ...
        if(node /*&& node.layer*/) {
            ...
            if (tree.getSelectionModel().getSelectedNode() === node) {
                ...
                if(!node.layer){
                    ...
                    renameFolderGroupItem.enable(); 
                    removeFolderGroupItem.enable();                               
                    if((node instanceof GeoExt.tree.LayerContainer))
                    {} /*forbidding Layer Container have folder inside it*/
                    else
                    {
                        addGroupItem.enable();
                        addFolderItem.enable();                                

                    }
                }
                else{
                    addGroupItem.disable();  
                    renameFolderGroupItem.disable();                           
                    removeFolderGroupItem.disable();
                    if(node.leaf || node instanceof GeoExt.tree.LayerContainer)
                        addFolderItem.disable();                                
                }
